...
const Review = ({ setCurrentindex, setError, setRid }) => {
    ...
    const handeldelete = async (id) => {
        try {
            const res = await axios.post(`${url}/delreview`, { id });
            getdata()
    ...
    const handeledit = (id) => {
        setRid(id)
        setCurrentindex('editreviews')
    }
    return (
        <div className='my-3 container w-full px-4 space-y-4 '>
            <div className="w-full h-auto   ">

                <button onClick={() => setCurrentindex('addreview')} className='w-full p-4 border !border-gray-600 rounded-lg flex justify-between items-center '>
            ...
            <div className="lg:col-span-2 bg-gray-800 rounded-lg shadow-md  shadow-lg-gray-700">
                <div className="p-4 shadow-lg-b shadow-lg-gray-700 flex justify-between items-center">
                    <h3 className="font-medium">All Review</h3>
                    <button onClick={() => { setCurrentindex('addreview') }} className="nav-link text-cyan-400 hover:text-cyan-300 text-sm flex items-center">
