...
const Founder = ({ setCurrentindex, setError, error, setFid }) => {
    ...
    const handeldelete = async (id) => {
        ...
        try {
            ...
            const res = await axios.post(`${url}/delfounder`, { id });
            console.log(res)
    ...
    return (
        <div className='my-3 container w-full px-4 space-y-4 '>
            <div className="w-full h-auto   ">
                ...
                <button onClick={() => setCurrentindex('addfounder')} className='w-full p-4 border !border-gray-600 rounded-lg flex justify-between items-center '>
            ...
            <div className="lg:col-span-2 bg-gray-800 rounded-lg shadow-md  shadow-lg-gray-700">
                <div className="p-4 shadow-lg-b shadow-lg-gray-700 flex justify-between items-center">
                    <h3 className="font-medium">All Founder</h3>
                    <button onClick={() => { setCurrentindex('addfounder') }} className="nav-link text-cyan-400 hover:text-cyan-300 text-sm flex items-center">
                                        ...
                                        data.length > 0 && data.map((data) => (
                                            <>
                                                <tr className="shadow-lg-b  shadow-lg-gray-700">
                                                    <td className="px-4 py-3" dangerouslySetInnerHTML={{ __html: data.title }}></td>
                                                    <td className="px-4 py-3">{data.role}</td>
                                                    <td className="px-2 py-1 text-xs text-gray-300 p- rounded-md m-2">{data.descrp}</td>
