...
export default function Editin({ iid ,setCurrentindex}) {
    ...
    const handleSubmit = async (e) => {
        ...
        // Convert list2 [{key, value}] into [{key: value}]
        ...
        try {
            const res = await axios.post(`${url}/upindustry`, {
                id: iid,
                title,
                des,
                list,
    ...
    const getdata = async (id) => {
        try {
            const res = await axios.post(`${url}/getindustry`, {
                id
            });
            console.log(res)
            setData(res.data);
            ...
            const transformedList2 = (res.data.list2 || []).map(item => {
                const key = Object.keys(item)[0];
                return { key, value: item[key] };
            });

            setList2(transformedList2);
        } catch (error) {
